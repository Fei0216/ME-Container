...
func RunTCPServer(TCPRequesthandler func(c net.Conn, n int), conn_port string) {
...
func SendCmdAndPort(conn_host string, conn_port string, cmd string, port string) (string, string) {
	...
	fmt.Println("Connected to server ", conn_host, ":", conn_port)
	...
	_, err1 := conn.Read(cmdbuf)
	Check(err1, "Unable to read response cmd")

	_, err2 := conn.Read(buf)
	Check(err2, "Unable to read response buf")
