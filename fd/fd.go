...
func handleRequest() {
   ...
   // msg = RecvText(clientCon,  "string")
    ...
    fmt.Println("processedFileName", processedFileName)
...
func main() {
	...
	stn := st.UnixNano()
	stm := stn / 1000000
	fmt.Println("start time is: ", stm)

	//fmt.Println("Start Image Processing ")
	// Handle connections
	handleRequest()
	fmt.Println("End Image Processing ")
	et := time.Now()
	etn := et.UnixNano()
	etm := etn / 1000000
	time := etm - stm
	fmt.Println(" end time is: ", etm)
